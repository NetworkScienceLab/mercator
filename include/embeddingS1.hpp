            ...
            for (; it1 != end1; ++it1) {
                ...
                if (simulated_degree[*it1] > 1) {
                    ...
                    if (neighbors_v2.size() > 0) {
                        ...
                        it4 = std::set_intersection(simulated_adjacency_list[v1].begin(),
                                                    simulated_adjacency_list[v1].end(),
...
void embeddingS1_t::build_cumul_dist_for_mc_integration() {
    ...
    if (!NUMERIC_MU_MODE && beta >= 1) { mu = beta * std::sin(PI / beta) / (2.0 * PI * average_degree); } else if (
        !NUMERIC_MU_MODE && beta < 1) {
        mu = (1 - beta) * std::pow(2, -beta) * std::pow(nb_vertices, beta - 1) / average_degree;
    ...
    for (; it1 != end1; ++it1) {
        ...
        end2 = degree_class.end();
        for (; it2 != end2; ++it2) {
            tmp_val = hyp2f1a(beta, PI * R,
                              mu * random_ensemble_kappa_per_degree_class[*it1] * random_ensemble_kappa_per_degree_class
                              [*it2]);
            nkkp[*it2] = degree2vertices[*it2].size() * tmp_val / random_ensemble_expected_degree_per_degree_class[*
                             it1];
        }
...
void embeddingS1_t::rewire_atzero() {
    ...
    while (counter <= maxcounter) {
        ...
        while (nodei == nodej || nodei == nodel || nodei == nodem || nodej == nodem || nodej == nodel || nodel == nodem
               ||
               std::find(rewired_adjacency_list.at(nodei).begin(), rewired_adjacency_list.at(nodei).end(), nodem) !=
               rewired_adjacency_list.at(nodei).end() ||
               std::find(rewired_adjacency_list.at(nodel).begin(), rewired_adjacency_list.at(nodel).end(), nodej) !=
               rewired_adjacency_list.at(nodel).end()) {
        ...
        *(std::find(rewired_adjacency_list.at(nodel).begin(), rewired_adjacency_list.at(nodel).end(), nodem)) = nodej;
        ...
        if (counter % (10 * nb_edges) == 0) {
            ...
            for (int i = 0; i < nb_vertices; i++) {
                if (degree.at(i) > 1) {
                    for (vector<int>::iterator it1 = rewired_adjacency_list.at(i).begin();
                         it1 != rewired_adjacency_list.at(i).end(); ++it1) {
                        for (vector<int>::iterator it2 = (it1 + 1); it2 != rewired_adjacency_list.at(i).end(); ++it2) {
                            for (vector<int>::iterator it3 = rewired_adjacency_list.at(*it2).begin();
                                 it3 != rewired_adjacency_list.at(*it2).end(); ++it3) {
                                if (*it1 == *it3) { links++; }
                            }
                        }
                    }
    ...
    for (std::vector<double>::iterator it = clustering_realizations.begin(); it != clustering_realizations.end(); ++
         it) {
    ...
    if (Pbis0 < 0.01/*(average_clustering - mu)/std > 3*/) {
        ...
        if (!QUIET_MODE) {
            std::clog << "WARNING: EXTREMELY WEAK METRICITY, P(C > c | beta = 0) = " << Pbis0 << std::endl;
        };
        std::cerr << "Extremely weak geometry, P(C > c | beta = 0) = " << Pbis0 << std::endl;
        std::cerr << "Mercator unable to determine beta. Please rerun the program with a manually chosen beta." <<
                std::endl;
        std::cerr << std::endl;
        // std::terminate();
...
void embeddingS1_t::compute_inferred_ensemble_expected_degrees() {
    ...
    for (int v1(0); v1 < nb_vertices; ++v1) {
        ...
        for (int v2(v1 + 1); v2 < nb_vertices; ++v2) {
            ...
            if (beta >= 1) {
                prob = 1 / (1 + std::pow((nb_vertices * dtheta) / (2 * PI * mu * kappa1 * kappa[v2]), beta));
            }
            if (beta < 1) {
                prob = 1 / (1 + std::pow((nb_vertices * dtheta) / (2 * PI), beta) * 1.0 / (mu * kappa1 * kappa[v2]));
            }
...
double embeddingS1_t::compute_random_ensemble_clustering_for_degree_class(int d1) {
    ...
    if (!NUMERIC_MU_MODE && beta > 1) { mu = beta * std::sin(PI / beta) / (2.0 * PI * average_degree); } else if (
        !NUMERIC_MU_MODE && beta < 1) {
        mu = (1 - beta) * std::pow(2, -beta) * std::pow(nb_vertices, beta - 1) / average_degree;
    } else if (!NUMERIC_MU_MODE && beta == 1) { mu = 1.0 / (2 * average_degree * std::log(nb_vertices)); } else {
    ...
    for (int i(0); i < nb_points; ++i) {
        ...
        p12 = hyp2f1a(beta, PI * R,
                      mu * random_ensemble_kappa_per_degree_class[d1] * random_ensemble_kappa_per_degree_class[d2]);

        // Gets the degree of vertex 3;
        d3 = cumul_prob_kgkp[d1].lower_bound(uniform_01(engine))->second;
        // Computes their probability of being connected.
        p13 = hyp2f1a(beta, PI * R,
                      mu * random_ensemble_kappa_per_degree_class[d1] * random_ensemble_kappa_per_degree_class[d3]);
        ...
        while ((zmax - zmin) > NUMERICAL_CONVERGENCE_THRESHOLD_2) {
            z = (zmax + zmin) / 2;
            pz = (z / PI) * hyp2f1a(beta, z * R,
                                    mu * random_ensemble_kappa_per_degree_class[d1] *
                                    random_ensemble_kappa_per_degree_class[d2]) / p12;
            if (pz > pc) {
                zmax = z;
            } else {
                zmin = z;
        ...
        while ((zmax - zmin) > NUMERICAL_CONVERGENCE_THRESHOLD_2) {
            ...
            pz = (z / PI) * hyp2f1a(beta, z * R,
                                    mu * random_ensemble_kappa_per_degree_class[d1] *
                                    random_ensemble_kappa_per_degree_class[d3]) / p13;
            if (pz > pc) {
                zmax = z;
            } else {
                zmin = z;
        ...
        if (da < NUMERICAL_ZERO) {
            ...
            if (beta >= 1) {
                p23 += 1.0 / (1.0 + std::pow(
                                  (da * R) / (mu * random_ensemble_kappa_per_degree_class[d2] *
                                              random_ensemble_kappa_per_degree_class[d3]), beta));
            }
            if (beta < 1) {
                p23 += 1.0 / (1.0 + std::pow((da * R), beta) * 1.0 / (
                                  mu * random_ensemble_kappa_per_degree_class[d2] *
                                  random_ensemble_kappa_per_degree_class[d3]));
            }
        }
    }
...
double embeddingS1_t::compute_pairwise_loglikelihood(int v1, double t1, int v2, double t2, bool neighbors) {
    ...
    } else // not neighbors
    {
        if (beta >= 1) {
            ...
            return -1 * std::log(1 + std::pow((nb_vertices * da) / (2 * PI), -beta) * mu * kappa[v1] * kappa[v2]);
        }
    }
}
...
void embeddingS1_t::finalize() {
    ...
    if (!QUIET_MODE) {
        std::clog << TAB << "BETA_ABS_MIN_GEOMETRIC                 " << BETA_ABS_MIN_GEOMETRIC << std::endl;
    }
    if (!QUIET_MODE) {
        std::clog << TAB << "BETA_ABS_MIN_NON_GEOMETRIC             " << BETA_ABS_MIN_NON_GEOMETRIC << std::endl;
    }
    if (!QUIET_MODE) {
    ...
    if (!QUIET_MODE) {
        std::clog << TAB << "CUSTOM_CHARACTERIZATION_NB_GRAPHS      " << (CUSTOM_CHARACTERIZATION_NB_GRAPHS
                                                                              ? "true"
                                                                              : "false") << std::endl;
    }
    if (!QUIET_MODE) {
        std::clog << TAB << "CUSTOM_INFERRED_COORDINATES            " << (
            CUSTOM_INFERRED_COORDINATES ? "true" : "false") << std::endl;
    }
    if (!QUIET_MODE) {
        std::clog << TAB << "CUSTOM_OUTPUT_ROOTNAME_MODE            " << (
            CUSTOM_OUTPUT_ROOTNAME_MODE ? "true" : "false") << std::endl;
    }
    if (!QUIET_MODE) {
    ...
    if (!QUIET_MODE) {
        std::clog << TAB << "METRICITY_TEST_NB_GRAPHS               " << METRICITY_TEST_NB_GRAPHS << std::endl;
    }
    ...
    if (!QUIET_MODE) {
        std::clog << TAB << "ALL_BETA_MODE                          " << (ALL_BETA_MODE ? "true" : "false") <<
                std::endl;
    }
    if (!QUIET_MODE) {
        std::clog << TAB << "NUMERIC_MU_MODE                        " << (NUMERIC_MU_MODE ? "true" : "false") <<
                std::endl;
    }
    if (!QUIET_MODE) {
    ...
    if (!REFINE_MODE)
        if (!QUIET_MODE) {
            std::clog << TAB << "parameters inference:                " << std::setw(10) << std::fixed << time2 - time1
                    << " seconds" << std::endl;
        }
    if (!REFINE_MODE)
        if (!QUIET_MODE) {
            std::clog << TAB << "initial positions:                   " << std::setw(10) << std::fixed << time3 - time2
                    << " seconds" << std::endl;
        }
    if (REFINE_MODE)
        if (!QUIET_MODE) {
            std::clog << TAB << "loading previous positions:          " << std::setw(10) << std::fixed << time3 - time1
                    << " seconds" << std::endl;
        }
    if (MAXIMIZATION_MODE)
        if (!QUIET_MODE) {
            std::clog << TAB << "refining positions:                  " << std::setw(10) << std::fixed << time4 - time3
                    << " seconds" << std::endl;
        }
    if (!QUIET_MODE) {
        std::clog << TAB << "adjusting kappas:                    " << std::setw(10) << std::fixed << time5 - time4 <<
                " seconds" << std::endl;
    }
    if (VALIDATION_MODE || CHARACTERIZATION_MODE)
        if (!QUIET_MODE) {
            std::clog << "Elapsed CPU time (validation):           " << std::setw(10) << std::fixed << time7 - time5 <<
                    " seconds" << std::endl;
        }
    if (VALIDATION_MODE)
        if (!QUIET_MODE) {
            std::clog << TAB << "validating embedding:                " << std::setw(10) << std::fixed << time6 - time5
                    << " seconds" << std::endl;
        }
    if (CHARACTERIZATION_MODE)
        if (!QUIET_MODE) {
            std::clog << TAB << "characterizing ensemble:             " << std::setw(10) << std::fixed << time7 - time6
                    << " seconds" << std::endl;
        }
    if (!QUIET_MODE) {
        std::clog << "===========================================================================================" <<
...
void embeddingS1_t::find_initial_ordering(std::vector<int> &ordering, std::vector<double> &raw_theta) {
    ...
    while (keep_going) {
        ...
        // Initializes and computes.
        if (!QUIET_MODE) {
            std::clog << TAB << "Computing eigenvectors using Spectra parameter ncv = " << ncv << "...";
        }
        eigs.init();
        int nconv = eigs.compute();
        ...
        if (eigs.info() != Spectra::SUCCESSFUL) {
            ...
            if (ncv == nb_vertices) {
                std::cerr << std::endl <<
                        "The algorithm computing the eigenvectors (Spectra library) cannot converge at all... Exiting."
                        << std::endl << std::endl;
                // std::terminate();
...
void embeddingS1_t::infer_initial_positions() {
    ...
    if (ordering.size() != nb_vertices)
        std::cout << TAB <<
                "WARNING: All degree-one vertices have not all been reinserted. Does the original edgelist have more than one connected component."
                << std::endl;
    ...
    for (; it != end; ++it, ++n) {
        ...
        if (beta >= 1) {
            tmp = std::exp(-PI / avg_gap) / (1 + std::pow(
                                                 nb_vertices / (
                                                     2.0 * mu * random_ensemble_kappa_per_degree_class[degree[v0]] *
                                                     random_ensemble_kappa_per_degree_class[degree[v1]]), b));
        }
        if (beta < 1) {
            tmp = std::exp(-PI / avg_gap) / (1 + std::pow(nb_vertices / 2.0, b) * std::pow(
                                                 mu * random_ensemble_kappa_per_degree_class[degree[v0]] *
                                                 random_ensemble_kappa_per_degree_class[degree[v1]], -sign));
        }
        int1 += PI * tmp / 2;
        int2 += tmp / 2;
        // In-between points.
        for (double da = dx; da < PI; da += dx) {
            if (beta >= 1) {
                tmp = std::exp(-da / avg_gap) / (1 + std::pow(
                                                     nb_vertices * da / (
                                                         2.0 * PI * mu * random_ensemble_kappa_per_degree_class[degree[
                                                             v0]] * random_ensemble_kappa_per_degree_class[degree[v1]]),
                                                     b));
            }
            if (beta < 1) {
                tmp = std::exp(-da / avg_gap) / (1 + std::pow(nb_vertices * da / (2.0 * PI), b) * std::pow(
                                                     mu * random_ensemble_kappa_per_degree_class[degree[v0]] *
                                                     random_ensemble_kappa_per_degree_class[degree[v1]], -sign));
            }
            int1 += da * tmp;
    ...
    if (!QUIET_MODE) {
        std::clog << std::endl << TAB << "Sum of the angular positions (before adjustment): " << norm << std::endl;
    }
...
void embeddingS1_t::infer_kappas_given_beta_for_all_vertices() {
    ...
    if (cnt >= KAPPA_MAX_NB_ITER_CONV) {
        if (!QUIET_MODE) {
            std::clog << TAB << "WARNING: maximum number of iterations reached before convergence. This limit can be" <<
                    std::endl;
        }
        if (!QUIET_MODE) {
            std::clog << TAB << "         adjusted by setting the parameters KAPPA_MAX_NB_ITER_CONV to desired value."
                    << std::endl;
        }
    } else {
...
void embeddingS1_t::infer_kappas_given_beta_for_degree_class() {
    ...
    while (keep_going && (cnt < KAPPA_MAX_NB_ITER_CONV)) {
        ...
        for (; it1 != end; ++it1) {
            it2 = it1;
            prob_conn = hyp2f1a(beta, nb_vertices / 2.0,
                                mu * random_ensemble_kappa_per_degree_class[*it1] *
                                random_ensemble_kappa_per_degree_class[*it2]);
        ...
        for (; it1 != end; ++it1) {
            if (std::fabs(random_ensemble_expected_degree_per_degree_class[*it1] - *it1) >
        ...
        if (keep_going) {
            ...
            for (; it1 != end; ++it1) {
                random_ensemble_kappa_per_degree_class[*it1] += (
                    *it1 - random_ensemble_expected_degree_per_degree_class[*it1]) * uniform_01(engine);
...
void embeddingS1_t::calculate_numerical_mu() {
    ...
    while (keep_going && cnt < FINDING_MU_STEPS) {
        ...
        for (it1 = degree_class.begin(); it1 != degree_class.end(); ++it1) {
            ...
            for (it2 = degree_class.begin(); it2 != degree_class.end(); ++it2) {
                ...
                if (beta > 1) {
                    ...
                    int2 += 1. / nb_vertices * Nk1 * Nk2 * 1. / (
                        1 + std::pow(nb_vertices * 1. / 2, beta) / (mu * k1 * k2));
                } else {
        ...
        if (std::abs(int1 - average_degree) < NUMERICAL_CONVERGENCE_THRESHOLD_2) {
            ...
            if (beta >= 1) { mu -= mu * (int1 - average_degree) / (int1 - int2); } else if (beta > 0) {
                mu -= beta * mu * (int1 - average_degree) / (int1 - int2);
    ...
    if (cnt >= FINDING_MU_STEPS) {
        ...
        if (!QUIET_MODE) {
            std::clog << TAB << "         adjusted by setting the parameters FINDING_MU_STEPS to desired value. Using"
                    << std::endl;
        }
...
void embeddingS1_t::infer_parameters() {
    ...
    if (!CUSTOM_BETA) {
        ...
        if (!ALL_BETA_MODE) { beta_min = 1; } else { beta_min = 0; }
        ...
        while (true) {
            ...
            if (std::fabs(random_ensemble_average_clustering - average_clustering) <
                NUMERICAL_CONVERGENCE_THRESHOLD_1) {
                break;
            }
            ...
            if (random_ensemble_average_clustering > average_clustering) {
                ...
                if (!ALL_BETA_MODE && beta < BETA_ABS_MIN_GEOMETRIC) {
                    if (!QUIET_MODE) {
                        std::clog << "WARNING: value too close to 1, using beta = " << std::fixed << std::setw(11) <<
                                beta << ".";
                    }
                    if (!QUIET_MODE) { std::clog << std::endl; }
                    break;
                ...
                if (ALL_BETA_MODE && beta < BETA_ABS_MIN_NON_GEOMETRIC) {
                    if (!QUIET_MODE) {
                        std::clog << "WARNING: value too close to 0, using beta = " << std::fixed << std::setw(11) <<
                                beta << ".";
                    }
                    if (!QUIET_MODE) { std::clog << std::endl; }
                    break;
    ...
    if (!QUIET_MODE) {
        std::clog << TAB << "Minimum degree:                 " << random_ensemble_expected_degree_per_degree_class.
                begin()->first << std::endl;
    }
    if (!QUIET_MODE) {
        std::clog << TAB << "Maximum degree:                 " << (--random_ensemble_expected_degree_per_degree_class.
            end())->first << std::endl;
    }
    if (!QUIET_MODE) {
...
void embeddingS1_t::initialize() {
    ...
    if (REFINE_MODE) {
        if (!QUIET_MODE) {
            std::clog << "inferred positions filename: " << ALREADY_INFERRED_PARAMETERS_FILENAME << std::endl;
        }
    }
...
void embeddingS1_t::refine_positions() {
    ...
    for (int v_i(0), v_f(0), v_m, n_v; v_f < nb_vertices;) {
        ...
        if (!QUIET_MODE) {
            std::clog << "...done in " << std::setw(6) << std::fixed << stop_time - start_time << " seconds (" <<
                    std::setw(std::log10(delta_nb_vertices) + 1) << v_m << "/" << std::setw(
                        std::log10(delta_nb_vertices) + 1) << n_v << " changed position)" << std::endl;
        }
    }
    // for(int j(0); j<5; ++j)
    // {
...
void embeddingS1_t::save_inferred_connection_probability() {
    ...
    if (!QUIET_MODE) { std::clog << std::endl; }
    if (!QUIET_MODE) {
        std::clog << TAB << "=> Inferred connection probability saved to " << ROOTNAME_OUTPUT + ".inf_pconn" <<
                std::endl;
    }
}
...
void embeddingS1_t::save_inferred_coordinates() {
    ...
    coordinates_file << "# " << TAB << "ALREADY_INFERRED_PARAMETERS_FILENAME   " << ALREADY_INFERRED_PARAMETERS_FILENAME
            << std::endl;
    ...
    coordinates_file << "# " << TAB << "BETA_ABS_MIN_NON_GEOMETRIC             " << BETA_ABS_MIN_NON_GEOMETRIC <<
            std::endl;
    coordinates_file << "# " << TAB << "CHARACTERIZATION_MODE                  " << (
        CHARACTERIZATION_MODE ? "true" : "false") << std::endl;
    coordinates_file << "# " << TAB << "CHARACTERIZATION_NB_GRAPHS             " << CHARACTERIZATION_NB_GRAPHS <<
            std::endl;
    coordinates_file << "# " << TAB << "CLEAN_RAW_OUTPUT_MODE                  " << (
        CLEAN_RAW_OUTPUT_MODE ? "true" : "false") << std::endl;
    // coordinates_file << "# " << TAB << "CLOSE_ANGULAR_RANGE_FACTOR             " << CLOSE_ANGULAR_RANGE_FACTOR                       << std::endl;
    coordinates_file << "# " << TAB << "CUSTOM_BETA                            " << (CUSTOM_BETA ? "true" : "false") <<
            std::endl;
    coordinates_file << "# " << TAB << "CUSTOM_INFERRED_COORDINATES            " << (CUSTOM_INFERRED_COORDINATES
        ? "true"
        : "false") << std::endl;
    coordinates_file << "# " << TAB << "CUSTOM_OUTPUT_ROOTNAME_MODE            " << (CUSTOM_OUTPUT_ROOTNAME_MODE
        ? "true"
        : "false") << std::endl;
    coordinates_file << "# " << TAB << "CUSTOM_SEED                            " << (CUSTOM_SEED ? "true" : "false") <<
            std::endl;
    coordinates_file << "# " << TAB << "EDGELIST_FILENAME:                     " << EDGELIST_FILENAME << std::endl;
    coordinates_file << "# " << TAB << "EXP_CLUST_NB_INTEGRATION_MC_STEPS      " << EXP_CLUST_NB_INTEGRATION_MC_STEPS <<
            std::endl;
    coordinates_file << "# " << TAB << "EXP_DIST_NB_INTEGRATION_STEPS          " << EXP_DIST_NB_INTEGRATION_STEPS <<
            std::endl;
    ...
    coordinates_file << "# " << TAB << "KAPPA_POST_INFERENCE_MODE              " << (KAPPA_POST_INFERENCE_MODE
        ? "true"
        : "false") << std::endl;
    ...
    coordinates_file << "# " << TAB << "MAXIMIZATION_MODE                      " << (
        MAXIMIZATION_MODE ? "true" : "false") << std::endl;
    coordinates_file << "# " << TAB << "METRICITY_TEST_NB_GRAPHS               " << METRICITY_TEST_NB_GRAPHS <<
            std::endl;
    ...
    coordinates_file << "# " << TAB << "NUMERICAL_CONVERGENCE_THRESHOLD_1      " << NUMERICAL_CONVERGENCE_THRESHOLD_1 <<
            std::endl;
    coordinates_file << "# " << TAB << "NUMERICAL_CONVERGENCE_THRESHOLD_2      " << NUMERICAL_CONVERGENCE_THRESHOLD_2 <<
            std::endl;
    coordinates_file << "# " << TAB << "NUMERICAL_CONVERGENCE_THRESHOLD_3      " << NUMERICAL_CONVERGENCE_THRESHOLD_3 <<
            std::endl;
    coordinates_file << "# " << TAB << "NUMERICAL_ZERO                         " << NUMERICAL_ZERO << std::endl;
    coordinates_file << "# " << TAB << "QUIET_MODE                             " << (QUIET_MODE ? "true" : "false") <<
            std::endl;
    coordinates_file << "# " << TAB << "REFINE_MODE                            " << (REFINE_MODE ? "true" : "false") <<
            std::endl;
    ...
    coordinates_file << "# " << TAB << "VALIDATION_MODE                        " << (VALIDATION_MODE ? "true" : "false")
            << std::endl;
    coordinates_file << "# " << TAB << "ALL_BETA_MODE                          " << (ALL_BETA_MODE ? "true" : "false")
            << std::endl;
    coordinates_file << "# " << TAB << "NUMERIC_MU_MODE                        " << (NUMERIC_MU_MODE ? "true" : "false")
            << std::endl;
    coordinates_file << "# " << TAB << "VERBOSE_MODE                           " << (VERBOSE_MODE ? "true" : "false") <<
            std::endl;
    ...
    if (!QUIET_MODE) { std::clog << std::endl; }
    if (!QUIET_MODE) {
        std::clog << TAB << "=> Inferred coordinates saved to " << ROOTNAME_OUTPUT + ".inf_coord" << std::endl;
    }
    ...
    if (CLEAN_RAW_OUTPUT_MODE) {
        ...
        if (!QUIET_MODE) { std::clog << std::endl; }
        if (!QUIET_MODE) {
            std::clog << TAB << "=> Raw inferred coordinates also saved to " << ROOTNAME_OUTPUT + ".inf_coord_raw" <<
                    std::endl;
        }
    }
    ...
    if (warning) {
        if (!QUIET_MODE) {
            std::clog << "WARNING: Hyperbolic radius has been adjusted to account for negative radial positions." <<
                    std::endl;
        }
    }
}
...
void embeddingS1_t::save_inferred_ensemble_characterization() {
    ...
    if (!CUSTOM_CHARACTERIZATION_NB_GRAPHS) {
        if (nb_vertices < 500) { CHARACTERIZATION_NB_GRAPHS = 1000; } else if (
            nb_vertices < 1000) { CHARACTERIZATION_NB_GRAPHS = 500; } else if (nb_vertices < 10000) {
            CHARACTERIZATION_NB_GRAPHS = 100;
        } else { CHARACTERIZATION_NB_GRAPHS = 10; }
    }
    if (!QUIET_MODE) { std::clog << std::endl; }
    if (!QUIET_MODE) {
        std::clog << TAB << "A total of " << CHARACTERIZATION_NB_GRAPHS <<
                " graphs will be generated (chosen in function of the total number" << std::endl;
    }
    if (!QUIET_MODE) {
        std::clog << TAB << "of vertices). To change this value, set the flag 'CUSTOM_CHARACTERIZATION_NB_GRAPHS'" <<
                std::endl;
    }
    if (!QUIET_MODE) {
        std::clog << TAB << "to 'true' and set the variable 'CHARACTERIZATION_NB_GRAPHS' to the desired value." <<
                std::endl;
    }
    if (!QUIET_MODE) { std::clog << std::endl; }
    ...
    for (int i(0); i < characterizing_inferred_ensemble_vprops.size(); ++i) {
        for (int v1(0); v1 < nb_vertices; ++v1) {
            ...
            characterizing_inferred_ensemble_vprops[i][v1][1] -= characterizing_inferred_ensemble_vprops[i][v1][0] *
                    characterizing_inferred_ensemble_vprops[i][v1][0];
            characterizing_inferred_ensemble_vprops[i][v1][1] *= CHARACTERIZATION_NB_GRAPHS / (
                CHARACTERIZATION_NB_GRAPHS - 1);
            characterizing_inferred_ensemble_vprops[i][v1][1] = std::sqrt(
                characterizing_inferred_ensemble_vprops[i][v1][1]);
        }
    }
    ...
    for (int norm, d; it3 != end3; ++it3) {
        ...
        for (int i(0); i < 10; ++++i) {
            ...
            if (norm > 1) {
                ...
                characterizing_inferred_ensemble_vstat[d][i + 1] -= characterizing_inferred_ensemble_vstat[d][i + 0] *
                        characterizing_inferred_ensemble_vstat[d][i + 0];
                ...
                if (characterizing_inferred_ensemble_vstat[d][i + 1] < 0) {
                    ...
                    characterizing_inferred_ensemble_vstat[d][i + 1] = std::sqrt(
                        characterizing_inferred_ensemble_vstat[d][i + 1]);
                }
            } else {
                characterizing_inferred_ensemble_vstat[d][i + 1] = 0;
            }
    ...
    if (!QUIET_MODE) { std::clog << std::endl; }
    if (!QUIET_MODE) {
        std::clog << TAB << "=> Vertices properties of the inferred ensemble saved to " << ROOTNAME_OUTPUT +
                ".inf_vprop" << std::endl;
    }
    ...
    if (!QUIET_MODE) { std::clog << std::endl; }
    if (!QUIET_MODE) {
        std::clog << TAB << "=> Inferred ensemble statistics by degree class saved to " << ROOTNAME_OUTPUT +
                ".inf_vstat" << std::endl;
    }
    ...
    if (!QUIET_MODE) { std::clog << std::endl; }
    if (!QUIET_MODE) {
        std::clog << TAB << "=> Original graph statistics by degree class saved to " << ROOTNAME_OUTPUT + ".obs_vstat"
                << std::endl;
    }
    // // Gets the current time.
    // time2 = std::time(NULL);
...
void embeddingS1_t::save_inferred_theta_density() {
    ...
    if (!QUIET_MODE) { std::clog << std::endl; }
    if (!QUIET_MODE) {
        std::clog << TAB << "=> Inferred theta density saved to " << ROOTNAME_OUTPUT + ".inf_theta_density" <<
                std::endl;
    }
}
...
void embeddingS1_t::check_connected_components() {
    ...
    if (lcc_size != nb_vertices) {
        ...
        if (!QUIET_MODE) {
            std::clog << TAB << "- " << lcc_size << "/" << nb_vertices << " vertices in the largest component." <<
                    std::endl;
        }
        std::cerr << std::endl;
        std::cerr << "More than one component found (" << lcc_size << "/" << nb_vertices <<
                ") vertices in the largest component." << std::endl;
        ...
        if (!QUIET_MODE) {
            std::clog << TAB << "- Edges belonging to the largest component saved to " << edgelist_rootname +
                    "_GC.edge." << std::endl;
        }
        ...
        if (QUIET_MODE) { std::clog << std::endl; }
        std::cerr << "Edges belonging to the largest component saved to " << edgelist_rootname +
                "_GC.edge. Please rerun the program using this new edgelist." << std::endl;
        std::cerr << std::endl;
        // std::terminate();
        throw std::runtime_error(
            "More than one component found. Please rerun the program using the largest component only.");
    }
}
